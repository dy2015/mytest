...
/**
 * 分组思路： 1、求得平均值 avg = sum/n (四舍五入)。 2、从右到左，依次与平均值
 * avg进行比较，数本身大于或者多个数相加之和等于平均值avg,则分为一组。
 * 3、将（剩下的数本身或者剩下的某几个数相加之和）与平均值avg之差的绝对值最小的，分为一组。 4、将被分的数的位置置为-2。同时，剩下未被分组的数为一组。
