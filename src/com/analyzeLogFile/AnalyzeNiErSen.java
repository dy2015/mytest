...
public class AnalyzeNiErSen {
	private final static String url = "C://Users//yt//Downloads//niersen.txt";
	private final static String cityUrl = "C://Users//yt//Downloads//niersenCity.txt";
	private final static String writeUrl = "C://Users//yt//Downloads//writeniersen.txt";
	...
	public void campareMap() {
		...
		for (int i = 0; i < size; i++) {
			...
			for (String key : Analyze.routMap.keySet()) {
				if (IpUtil.ipExistsInRange(index, key + "-" + Analyze.routMap.get(key))) {
					cityIDIPA = Analyze.routCityMap.get(key);
					break;
				}
			}
			list.add(index + " --> " + map.get(index) + " --> 尼尔森地域："+niersenipCitymap.get(index)+"---> 优酷地域："+Analyze.areaMap.get(cityIDIPA)+"\n");
			myipCitymap.put(index, Analyze.areaMap.get(cityIDIPA));
			map.remove(index);
		} // end--for
	}
